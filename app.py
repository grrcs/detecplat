...
class PlateGuardianApp(QMainWindow):
    ...
    #
    #  Dilakukan pada CROP PLAT (bukan gambar full).
    #  Setiap tahap ditampilkan di panel GUI.
    ...
    def preprocess_plat(self, plat_bgr):
        ...
        # TAHAP 1: GRAYSCALE
        # ----------------------------------------------------------
        # KENAPA: Citra plat memiliki 3 channel warna (Blue, Green, Red).
        # Untuk memisahkan karakter dari background, warna tidak relevan
        # - kita hanya butuh perbedaan intensitas cahaya antara huruf
        # (gelap) dan background plat (terang). Konversi ke 1 channel
        # menyederhanakan proses selanjutnya.
        # ----------------------------------------------------------
        ...
        # ----------------------------------------------------------
        # KENAPA: Potongan plat masih mengandung noise dari sensor
        # kamera dan artefak kompresi JPEG. Noise berupa variasi
        # piksel acak yang bisa membuat threshold menghasilkan
        # titik-titik hitam/putih palsu di sekitar karakter.
        #
        # Gaussian Blur kernel (5,5) menghaluskan noise menggunakan
        # distribusi Gaussian - piksel pusat berbobot besar, piksel
        # tepi berbobot kecil. Hasilnya: permukaan plat lebih halus
        # tapi tepi karakter tetap terjaga.
        # ----------------------------------------------------------
        blur = cv2.GaussianBlur(gray, (5, 5), 0)
        ...
        # ----------------------------------------------------------
        # KENAPA: Ditampilkan untuk memvisualisasikan tepi karakter
        # yang berhasil terdeteksi pada potongan plat. Canny menggunakan
        ...
        #
        # Tahap ini ditampilkan sebagai visualisasi - menunjukkan
        # bahwa tepi setiap karakter berhasil terdeteksi dengan jelas.
        # ----------------------------------------------------------
        edges = cv2.Canny(blur, 100, 200)
        ...
        # ----------------------------------------------------------
        # KENAPA: Untuk segmentasi karakter, kita butuh gambar binary
        # (hitam-putih tegas) - karakter = putih, background = hitam.
        #
        # Otsu Threshold menentukan nilai threshold secara OTOMATIS
        # dengan mencari nilai yang memaksimalkan variansi antar kelas
        # (karakter vs background). Tidak perlu menebak threshold
        # manual karena Otsu menghitung dari histogram gambar.
        #
        # THRESH_BINARY_INV digunakan karena karakter pada plat
        # berwarna gelap di atas background terang - inverse membuat
        # karakter menjadi putih (foreground) untuk findContours.
        # ----------------------------------------------------------
